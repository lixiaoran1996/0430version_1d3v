...
contains
   ...
   subroutine CS_read_file(filename, gas_name, x_normalization, y_normalization, req_energy)
      ...
      do
         ...
         case ("EFFECTIVE")
            col_type = CS_elastic_t
            write(err_string, *) "CS_read_file warning: using EFFECTIVE elastic collision cross section for ", &
         ...
         case ("COMMENT")
            cycle
         case DEFAULT
            write(err_string, *) "CS_read_file warning: ignoring unknown process type for ", &
         ...
         do
            ...
            if ( line(1:5) == "-----" ) then
               ...
               write(err_string, *) "CS_read_file error: too many rows in ", filename, " at line ", nL
               call ERR_show(err_string)
            end if
         end do

         if (n_rows < 2) then
            write(err_string, *) "CS_read_file error: need at least two values in ", &
         ...
         if ( CS_table(cIx)%en_cs(1, n_rows) < req_energy .and. &
            ...
            write(err_string, *) "CS_read_file error: cross section data at line ", nL, &
...
555   continue ! Routine ends here if the end of "filename" is reached erroneously
      ...
      write(err_string, *) "CS_read_file error, reached end of file while searching. ", &
...
999   continue ! If there was an error, the routine will end here
      write(err_string, *) "CS_read_file error at line ", nL, &
...
999   continue ! If there was an error, the routine will end here
      write(err_string, *) "CS_write_all error, io_state = ", io_state, " while writing to ", filename
      call ERR_show(err_string)
...
999   continue ! If there was an error, the routine will end here
      write(err_string, *) "CS_write_summary error, io_state = ", io_state, " while writing to ", filename
      call ERR_show(err_string)
