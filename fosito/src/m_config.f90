...
module m_config
   ...
   integer, parameter :: maxVarSize = 20
   ...
   integer  :: CFG_nVars = 0
   integer  :: CFG_maxVars = 0

   type(CFG_varType), pointer :: CFG_varList(:) => null()
...
contains
   ...
   subroutine CFG_read_file(filename)
      ...
      character(len=name_len)      :: pName
      character(len=line_len)      :: line, tempString
      ...
      do
         ...
         if (ix > 0) then
            ...
            if (CFG_varList(ix)%varSize) then
               ...
               call split_line_by_delims(line, " ,'"""//char(9), maxVarSize, nEntries, startIxs, endIxs)
            ...
            call ERR_show("CFG_read_file: variable [" // trim(pName) // &
...
998   continue
      call ERR_show("CFG_read_file: Not enough values for variable [" // trim(pName) // "] in " // filename)
      return

999   continue
      write(tempString, *) "ioState = ", ioState, " while reading from ", filename, " at line ", nL
      call ERR_show("CFG_read_file:" // tempString)
      return
   ...
   subroutine prepareStoreVar(pName, pType, pSize, comment, varSize)
      ...
      if (present(varSize)) then
         CFG_varList(CFG_nVars)%varSize = varSize
      else
      ...
      case (CFG_int_type)
         allocate( CFG_varList(CFG_nVars)%int_data(pSize) )
      case (CFG_real_type)
         allocate( CFG_varList(CFG_nVars)%real_data(pSize) )
      case (CFG_char_type)
         allocate( CFG_varList(CFG_nVars)%char_data(pSize) )
      case (CFG_logic_type)
         allocate( CFG_varList(CFG_nVars)%logic_data(pSize) )
      end select
   ...
   integer function prepareGetVar(pName, pType, pSize)
      ...
      if (ix == -1) then
         call ERR_show("CFG_get(something): variable ["//trim(pName)//"] not found")
      else if (.not. correctType) then
         call ERR_show("CFG_get(something): variable ["//trim(pName)//"] has different type")
      else if (.not. correctSize) then
         call ERR_show("CFG_get(something): variable["//trim(pName)//"] has different size")
      end if
   ...
   subroutine CFG_addReal(pName, real_data, comment)
      ...
      CFG_varList(CFG_nVars)%real_data(1) = real_data
   ...
   subroutine CFG_addRealArray(pName, real_data, comment, varSize)
      ...
      CFG_varList(CFG_nVars)%real_data = real_data
   ...
   subroutine CFG_addInt(pName, int_data, comment)
      ...
      CFG_varList(CFG_nVars)%int_data(1) = int_data
   ...
   subroutine CFG_addIntArray(pName, int_data, comment, varSize)
      ...
      CFG_varList(CFG_nVars)%int_data = int_data
   ...
   subroutine CFG_addChar(pName, char_data, comment)
      ...
      CFG_varList(CFG_nVars)%char_data(1) = char_data
   ...
   subroutine CFG_addCharArray(pName, char_data, comment, varSize)
      ...
      CFG_varList(CFG_nVars)%char_data = char_data
   ...
   subroutine CFG_addlogical(pName, logic_data, comment)
      ...
      CFG_varList(CFG_nVars)%logic_data(1) = logic_data
   ...
   subroutine CFG_addlogicalArray(pName, logic_data, comment, varSize)
      ...
      CFG_varList(CFG_nVars)%logic_data = logic_data
   ...
   subroutine prepare_get_var(pName, pType, argIndex, ix, pIndex)
      ...
      if (ix == -1) then
         ...
         call ERR_show("CFG_get(something): variable ["//trim(pName)//"] wrong or no index specified")
      end if
   ...
   integer function CFG_get_size(pName)
      ...
      if (ix /= -1) then
         ...
         CFG_get_size = -1
         call ERR_show("CFG_get_size: variable ["//trim(pName)//"] not found")
      end if
   ...
   integer function CFG_get_type(pName)
      ...
      if (ix /= -1) then
         ...
         CFG_get_type = -1
         call ERR_show("CFG_get_type: variable ["//trim(pName)//"] not found")
      end if
...
999   continue ! If there was an error, the routine will end here
   ...
   subroutine ensure_free_storage()
      ...
      type(CFG_varType), pointer :: cfg_copy(:)

      if (associated(CFG_varList)) then
         cur_size = size(CFG_varList)
         if (cur_size - CFG_nVars < 1) then
            new_size = 2 * cur_size
            allocate(cfg_copy(new_size))
            cfg_copy(1:CFG_nVars) = CFG_varList
            deallocate(CFG_varList)
   ...
   subroutine CFG_sort()
      call qsort_config(CFG_varList(1:CFG_nVars))
      CFG_sorted = .true.
