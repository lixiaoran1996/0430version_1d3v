...
module m_units_constants
  ...
  real(dp), parameter :: UC_eVToKelvin     = 1.1605d4                 ! 1eV = 11605 K = UC_elec_volt / UC_boltzmann_const
...
contains
  ...
  !anbang: here for testing Donko's rotation method,
  !> theta is the angle between velocity and x axis;
  !> phi is on y-z plane, the angle between y axis and the velocity
  subroutine UC_Donko_xyz_to_spherical(xyz, radius, theta, phi)
    real(dp), intent(in) :: xyz(3)
    real(dp), intent(out) :: radius, theta, phi

    if (xyz(2) == 0.0_dp .and. xyz(3) == 0.0_dp) then
       phi = 0.0_dp
    else
       phi = atan2(xyz(3), xyz(2))
    end if

    radius = sqrt(sum(xyz**2))
    if (radius /= 0.0_dp) then      ! Undefined
       theta = acos(xyz(1) / radius)
    else
       theta = 0.0_dp
    end if

 end subroutine UC_Donko_xyz_to_spherical
