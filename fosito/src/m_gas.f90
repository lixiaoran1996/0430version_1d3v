...
module m_gas
  ...
  character(len=tiny_len), allocatable :: GAS_comp_names(:)
...
contains
  ...
  subroutine GAS_initialize(comp_names, comp_fracs, pressure, temperature)
    ...
    print *, "GAS_number_dens =", GAS_number_dens
  ...
  real(dp) function GAS_get_fraction(comp_name)
    ...
    if (ix == GAS_num_gases + 1) call ERR_show("GAS_get_fraction: " // comp_name // " not found")
